...
<div class="admin-page-title">
  <div class="left">
    <h1>My Overview</h1>
    <p>Track your attendance, quizzes, and engagement</p>
  </div>
</div>
...
<div class="alert alert-warning">
  ...
  <span>You have <strong><?= $myAlerts ?> open alert<?= $myAlerts > 1 ? 's' : '' ?></strong>. Check your <a href="<?= APP_URL ?>/student/alerts.php">Alerts page</a> for details.</span>
</div>
<?php endif; ?>
...
<div class="stat-cards">
  ...
  <div class="card stat-card">
    <div class="stat-icon" style="background:#FFF7ED">
      <svg fill="none" viewBox="0 0 24 24" stroke="#F59E0B" stroke-width="2"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>
    </div>
    <div><div class="stat-value"><?= $avgEngage ?>%</div><div class="stat-label">Avg Engagement</div></div>
  </div>
</div>
...
<div class="card empty-state">
  <div class="icon-circle" style="background:#EFF6FF">
    <svg fill="none" viewBox="0 0 24 24" stroke="#2563EB" stroke-width="2"><path d="M22 10v6M2 10l10-5 10 5-10 5-10-5z"/><path d="M6 12v5c0 1.66 2.69 3 6 3s6-1.34 6-3v-5"/></svg>
  </div>
  <div class="title">No courses yet</div>
  Contact your Admin to be enrolled in a course.
</div>
<?php else: ?>
...
<?php foreach ($myCourses as $c):
  ...
  $present = $db->query(
    "SELECT COUNT(*) FROM attendance_records ar JOIN class_sessions cs ON ar.session_id=cs.id
     WHERE ar.student_id=? AND cs.course_id=? AND cs.status='ended' AND ar.status='present'", [$userId, $c['id']]
  )->fetchColumn();
  $total = $db->query("SELECT COUNT(*) FROM class_sessions WHERE course_id=? AND status='ended'", [$c['id']])->fetchColumn();
  $pct = $total > 0 ? min(100, round($present / $total * 100)) : 0;
  $ringColor = $eng >= 70 ? '#10B981' : ($eng >= 40 ? '#F59E0B' : '#EF4444');
?>
<div class="card card-interactive" style="padding:20px">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px">
    <div>
      ...
      <div style="font-weight:700;font-size:14px;color:#0F172A;margin-top:8px;line-height:1.3"><?= htmlspecialchars($c['course_name']) ?></div>
    </div>
    <div class="score-ring" style="--ring-size:60px;--pct:<?= $eng ?>;--ring-color:<?= $ringColor ?>">
      <span class="score-ring-label" style="font-size:13px"><?= round($eng) ?>%</span>
    </div>
  </div>
  <div style="font-size:12px;color:#64748B;margin-bottom:6px">Attendance: <?= $present ?>/<?= $total ?> sessions (<?= $pct ?>%)</div>
  <div class="progress-track" style="width:100%;margin-bottom:16px">
